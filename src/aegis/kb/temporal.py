...
class TemporalInterval(BaseModel):
    ...
    t_start: datetime | None = Field(default=None, description="Start of validity period.")
    t_end: datetime | None = Field(default=None, description="End of validity period (if known).")
...
class CausalActionNode(BaseModel):
    ...
    action_type: str = Field(..., description="Type of action (e.g., 'AMEND', 'SUPERSEDE').")
...
def point_in_time_filter(t: datetime | None = None) -> dict[str, Any]:
    ...
    return {
        "$and": [
            {"$or": [{"temporal_validity.t_start": {"$lte": t.isoformat()}}, {"temporal_validity.t_start": None}]},
            {"$or": [{"temporal_validity.t_end": {"$gt": t.isoformat()}}, {"temporal_validity.t_end": None}]}
        ]
    }

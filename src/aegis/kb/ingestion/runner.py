...
class IngestionRunner:
    ...
    def __init__(
        self, registry: SourceRegistry, state_manager: StateManager | None = None
    ) -> None:
        ...
        for name in extractor_names:
            ...
            if current_extracted is not None:
                current_bytes = json.dumps(
                    current_extracted.model_dump(), default=str
                ).encode("utf-8")
    ...
    async def run_source_pipeline(
        ...
        try:
            ...
            if not config.stages:
                ...
                async for raw_bytes, meta in connector.fetch(
                    config, checkpoint.state_data
                ):
                    extracted = await self._run_extractors(
                        extractor_names, raw_bytes, meta
                    )

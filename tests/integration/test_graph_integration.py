...
async def test_full_graph_execution_mocked():
    ...
    with patch(
        "aegis.graph.router._call_llama_cpp", new_callable=AsyncMock
    ) as mock_route_llm:
        ...
        with (
            patch(
                "aegis.graph.sql_flow._call_llm", new_callable=AsyncMock
            ) as mock_sql_llm,
            patch(
                "aegis.graph.sql_flow._embed_text", new_callable=AsyncMock
            ) as mock_embed,
...
async def test_full_graph_rag_path_mocked():
    ...
    with patch(
        ...
        with (
            patch("aegis.graph.privacy.RiskScorer.calculate_risk", return_value=0.01),
            patch(
                "aegis.graph.privacy.SemanticScrubber.scrub", side_effect=lambda x, m: x
            ),
            patch(
                "aegis.rag.retriever.Retriever.retrieve", new_callable=AsyncMock
            ) as mock_retrieve,
            patch(
                "aegis.common.cloud_llm.CloudLLMClient.generate", new_callable=AsyncMock
            ) as mock_gen,
            patch(
                "aegis.graph.staleness.check_staleness", new_callable=AsyncMock
            ) as mock_stale,
        ):
            mock_retrieve.return_value = [
                {"content": "info", "source": "s", "source_title": "t"}
            ]

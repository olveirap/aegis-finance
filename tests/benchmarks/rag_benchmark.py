...
class RagBenchmarkRunner:
    def __init__(self, db_url: str, llm_base_url: str, seed: int = 42, dry_run: bool = False):
        self.db_url = db_url
    ...
    async def run(self, dataset_path: str, output_path: str):
        ...
        try:
            ...
            conn = await asyncio.wait_for(psycopg.AsyncConnection.connect(self.db_url), timeout=5.0)
    ...
    async def _process_item(self, conn, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Embedder failed (llama.cpp likely not running): {e}. Using dummy embeddings.")
            q_emb = [0.0] * 1024
            ca_q_emb = [0.0] * 1024

        if conn is None:
            top_k_chunks = [{"content": "Dummy chunk from DB missing", "source": "dummy_source", "topic_tags": []}] * self.k_retrieval
            ca_top_k_chunks = [{"content": "Dummy empty DB chunk", "source": "dummy", "topic_tags": []}] * self.k_stuffed
        else:
            top_k_chunks = await self._retrieve_chunks(conn, q_emb, self.k_retrieval)
            ca_top_k_chunks = await self._retrieve_chunks(conn, ca_q_emb, self.k_stuffed)
        
        # 2. Layer Coverage @ 5
        ca_top_5 = ca_top_k_chunks[:self.k_retrieval]
        layer_coverage = self._evaluate_layer_coverage(ca_top_5, item.get("required_layers", {}))
        ...
        base_rel_array = await self._get_relevance_array(base_top_5, query, item.get("relevant_chunks", []))
        base_recall_at_5 = sum(base_rel_array) / len(base_rel_array) if base_rel_array else 0.0
        base_mrr = 0.0
        ...
        ca_rel_array = await self._get_relevance_array(ca_top_5, query, item.get("relevant_chunks", []))
        ...
        if stuffed_tokens > self.max_stuffed_tokens:
            logger.warning(f"Stuffed context exceeds {self.max_stuffed_tokens} tokens. Truncating.")
            stuffed_context = stuffed_context[:self.max_stuffed_tokens * 3] # rough char cut
            
        rag_answer = await self._generate_answer(user_context, query, rag_context)
        stuffed_answer = await self._generate_answer(user_context, query, stuffed_context)
        ...
        rag_score = eval_result["answer_a_score"] if is_rag_a else eval_result["answer_b_score"]
        stuffed_score = eval_result["answer_b_score"] if is_rag_a else eval_result["answer_a_score"]
        rag_hallucinated = eval_result["answer_a_hallucinated"] if is_rag_a else eval_result["answer_b_hallucinated"]
        ...
        status = "success"
        if conn is None or q_emb == [0.0] * 1024 or "Error" in rag_answer or "Error" in stuffed_answer:
    ...
    async def _retrieve_chunks(self, conn, embedding: List[float], k: int) -> List[Dict[str, Any]]:
        ...
        try:
            async with conn.cursor() as cur:
                ...
                if not res or not res[0]:
                    return [{"content": "Dummy chunk from DB missing", "source": "dummy_source"}] * k
                ...
                return [{"content": r[0], "source": r[1], "source_title": r[2], "topic_tags": r[3]} for r in rows]
        except psycopg.Error:
            return [{"content": "Mock retrieved content due to DB error", "source": "mock", "topic_tags": []}] * k

    def _evaluate_layer_coverage(self, chunks: List[Dict[str, Any]], required_layers: Dict[str, List[str]]) -> Dict[str, Any]:
        coverage = {}
        all_present = True
    ...
    async def _get_relevance_array(self, chunks, query, relevant_chunk_ids) -> List[bool]:
    ...
    async def _generate_answer(self, user_context: str, query: str, context_chunks: str) -> str:
        prompt = ANSWER_GENERATION_PROMPT.format(user_context=user_context, context_chunks=context_chunks, query=query)
        try:
            resp = await self.openai_client.chat.completions.create(
                model=self.llm_model,
                messages=[{"role": "user", "content": prompt}],
    ...
    async def _evaluate_answers(self, item: Dict[str, Any], ans_a: str, ans_b: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "answer_a_hallucinated": False, "answer_b_hallucinated": False,
                "answer_a_score": 0, "answer_b_score": 0, "reasoning": "Judge failed."
            }
            
    def _aggregate_metrics(self, metrics: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        aggs = {}
        for category, m_list in metrics.items():
            ...
            aggs[category] = {
                ...
                "layer_coverage_at_5": sum(1 for m in m_list if m["layer_coverage_all"]) / count,
                "avg_answer_quality_rag": sum(m["rag_score"] for m in m_list) / count,
                "hallucination_rate": sum(1 for m in m_list if m["rag_hallucinated"]) / count,
            }
        return {"by_query_type": {k: v for k, v in aggs.items() if k != "overall"}, "overall": aggs.get("overall", {})}

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run RAG Benchmarks")
    parser.add_argument("--db-url", type=str, default=os.environ.get("AEGIS_DB_URL", "postgresql://aegis:aegis_dev@localhost:5432/aegis_finance"))
    parser.add_argument("--llm-url", type=str, default="http://localhost:8080")
    parser.add_argument("--dataset", type=str, default="tests/benchmarks/kb_quality.json")
    parser.add_argument("--output", type=str, default="tests/benchmarks/results/kb_baseline.json")
